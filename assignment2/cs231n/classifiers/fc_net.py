...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        c1 ,c1_cache = affine_relu_forward(X,self.params['W1'],self.params['b1'])
        scores , final_cache = affine_forward(c1,self.params['W2'],self.params['b2'])
        ...
        dx, dw2, db2 = affine_backward(dx, final_cache)
        grads['W2'] = dw2 + self.reg*self.params['W2']
        grads['b2'] = db2

        dx, dw1, db1 = affine_relu_backward(dx,c1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
          if i == 0:
            self.params['W%s' %(i+1)] = np.random.normal(0,1,size = (input_dim,hidden_dims[0]))
            self.params['b%s' % (i+1)] = np.zeros(hidden_dims[0])
             if self.use_batchnorm:
               self.params['gamma1'] = np.ones(hidden_dims[0])
               self.params['beta1'] = np.zeros(hidden_dims[0])
          else:
            self.params['W%s' % (i+1)] = np.random.normal(0,1,size = (hidden_dims[i-1],hidden_dims[i]))
        ...
        self.params['W%s' % self.num_layers] = np.random.normal(0,1,size = (hidden_dims[-1],num_classes))
